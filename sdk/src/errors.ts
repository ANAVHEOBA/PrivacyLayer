 ...
 *   - PL_DEP_*   : Deposit-specific errors
 */
export enum ErrorCode {
  ...
  DEPOSIT_SIMULATION_FAILED = 'PL_DEP_001',
  /** Deposit transaction timed out waiting for confirmation */
  DEPOSIT_TIMEOUT = 'PL_DEP_002',
  /** Note generation failed (cryptographic error) */
  NOTE_GENERATION_FAILED = 'PL_DEP_003',
  /** Signer function was not provided */
  SIGNER_REQUIRED = 'PL_DEP_004',
  /** Contract ID was not provided or is invalid */
  INVALID_CONTRACT_ID = 'PL_DEP_005',
}
...
export const ERROR_MESSAGES: Record<ErrorCode, string> = {
  ...
  [ErrorCode.DEPOSIT_SIMULATION_FAILED]:
    'Deposit transaction simulation failed. The contract may be paused or the commitment invalid.',
  [ErrorCode.DEPOSIT_TIMEOUT]:
    'Deposit transaction timed out waiting for ledger confirmation.',
  [ErrorCode.NOTE_GENERATION_FAILED]:
    'Failed to generate a cryptographic note. Ensure the crypto provider is available.',
  [ErrorCode.SIGNER_REQUIRED]:
    'A signer function is required for deposit transactions. Provide a signTransaction callback.',
  [ErrorCode.INVALID_CONTRACT_ID]:
    'The contract ID is missing or invalid. Provide a valid Soroban contract address.',
};
...
/**
 * Errors specific to the deposit flow.
 */
export class DepositError extends PrivacyLayerError {
  /** The deposit phase that failed */
  public readonly phase?: 'note-generation' | 'build' | 'sign' | 'submit' | 'confirm';
  ...
  constructor(
    ...
    options?: {
      ...
      cause?: Error;
      isRetryable?: boolean;
      phase?: 'note-generation' | 'build' | 'sign' | 'submit' | 'confirm';
    },
  ) {
    super(code, message, {
      details: options?.details,
...
/** Type guard for DepositError */
export function isDepositError(err: unknown): err is DepositError {
  return err instanceof DepositError;
}
